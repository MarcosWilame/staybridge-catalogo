...
export function HowItWorks() {
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-[var(--gray-light)] to-white py-12 md:py-20">
      {/* Diagonal Background */}
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 md:gap-8 lg:grid-cols-4">
          {steps.map((step, index) => {
            ...
            return (
              <div
                key={index}
                className="group relative rounded-xl border-2 border-transparent bg-white p-5 shadow-lg transition-all duration-300 hover:border-[var(--green-dark)] hover:shadow-2xl md:rounded-2xl md:p-8"
              >
                {/* Number Badge */}
                <div className="absolute -right-3 -top-3 flex h-12 w-12 items-center justify-center rounded-full bg-[var(--yellow)] shadow-lg transition-transform duration-300 group-hover:scale-110 md:-right-4 md:-top-4 md:h-16 md:w-16">
                  <span className="text-lg font-bold text-black md:text-2xl">{step.number}</span>
                </div>
